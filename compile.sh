...
function build_snappy {
	...
	if cant_use_cache "$snappy_dir"; then
		rm -rf "$snappy_dir"
		write_download
		download_github_src "google/snappy" "refs/tags/$LIBSNAPPY_VER" "snappy" | tar -zx >> "$DIR/install.log" 2>&1
		write_configure
		...
		cmake .. \
			...
			>> "$DIR/install.log" 2>&1 || { echo "CMake failed"; exit 1; }
	...
	cd ../..
	if [ "$DO_STATIC" != "yes" ]; then
		rm -f "$INSTALL_DIR/lib/libsnappy.a"
	fi
	write_done
}
